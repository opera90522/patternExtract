...
# 5000 mixed-domain messages -> 66 templates in 0.78s (coverage 96.4%)
...
# 15000 messages in 1.70s = 8,814 msg/s (96.4% matched)
```
...
- `notebooks/approach.ipynb` walks the whole approach on the committed synthetic
  multi-domain corpus with outputs saved. It is generated by
  `notebooks/build_notebook.py` so it stays reviewable in diffs.
- `notebooks/full_pipeline.ipynb` is a single, end-to-end notebook that reads a
  CSV, learns templates, benchmarks matching, profiles learning/matching with
  `cProfile`, and reports coverage. Change `CSV_PATH` at the top to point it at
  your real data. Run it inside the included `.devcontainer/` image with no
  further downloads.
